...
-- Signature logos must be public so external email clients can load them.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
...
create policy signature_assets_insert_own on storage.objects for insert to authenticated with check (bucket_id = 'signature-assets' and (storage.foldername(name))[1] = auth.uid()::text);
drop policy if exists signature_assets_update_own on storage.objects;
create policy signature_assets_update_own on storage.objects for update to authenticated using (bucket_id = 'signature-assets' and (storage.foldername(name))[1] = auth.uid()::text) with check (bucket_id = 'signature-assets' and (storage.foldername(name))[1] = auth.uid()::text);
drop policy if exists signature_assets_delete_own on storage.objects;
create policy signature_assets_delete_own on storage.objects for delete to authenticated using (bucket_id = 'signature-assets' and (storage.foldername(name))[1] = auth.uid()::text);
